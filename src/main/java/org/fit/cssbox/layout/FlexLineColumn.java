...
/**
 * Flex line in vertical flex container
 *
 * @author Ondra, Ondry
 */
public class FlexLineColumn extends FlexLine {
    ...
    /**
     * Creates new instance of flex line in vertical flex container
     *
    ...
    protected void applyAlignContent(ArrayList<FlexLine> lines, int countOfPreviousLines) {
        ...
        if (owner.alignContent == FlexBox.ALIGN_CONTENT_FLEX_START) {
            ...
            setX(getX() + remainingWidthSpace / 2);
            ...
            if (lines.size() != 1)
                setX(getX() + (countOfPreviousLines) * remainingWidthSpace / (lines.size() - 1));
    ...
    /**
     * Auxiliary method for processing width
     *
     * @param item flex item
     */
